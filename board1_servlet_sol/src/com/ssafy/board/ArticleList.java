...
public class ArticleList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("              <button type=\"button\" id=\"btn-mv-register\" class=\"btn btn-outline-primary btn-sm\">글쓰기</button>");
		out.println("            </div>");
		out.println("            <div class=\"col-md-7 offset-3\">");
		out.println("              <form class=\"d-flex\">");
		out.println("                <select id=\"key\" class=\"form-select form-select-sm ms-5 me-1 w-50\" aria-label=\"검색조건\">");
		...
		ResultSet rs = null;
		try {
			conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/ssafyweb?serverTimezone=UTC", "ssafy", "1234");
			...
			while(rs.next()) {
				...
				out.println("                  <a href=\"#\" class=\"article-title link-dark\" data-no=\"" + rs.getInt("article_no") + "\" style=\"text-decoration: none\">");
		...
		out.println("    ></script>");
		out.println("    <script>");
		out.println("      let links = document.querySelectorAll(\".article-title\");");
		out.println("      links.forEach(function (link) {");
		out.println("        link.addEventListener(\"click\", function () {");
